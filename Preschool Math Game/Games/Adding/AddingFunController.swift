//
//  SubtractingFunController.swift
//  Preschool Math Game
//
...
class AddingFunController: BaseViewController {
    ...
    private var imagesArray = ["priz_img_1", "priz_img_5", "priz_img_6", "priz_img_7", "arjuk", "crar", "kanfet", "puchik"]
    ...
    static func initWithStoryboard() -> AddingFunController? {
        let storyboard = UIStoryboard(name: "AddingGames", bundle: nil)
        return storyboard.instantiateViewController(withIdentifier: AddingFunController.name) as? AddingFunController
    }
...
//        AnalyticService.shared.setupAmplitude(game: "start_adding_1")
        
        setBorders()
        
//        GamesLimit.addingPrizCount = UserDefaults.standard.value(forKey: UserDefaultsKeys.addingPrizCount) as? Int ?? 0
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(update), name: .updateAddingFun, object: nil)
    }
    ...
    @objc func gestureButtonAction(gesture: UIPanGestureRecognizer) {
        threeButtonPanGesture(gesture: gesture, gamesLimit: GamesLimit.addingFunLimit, resultSpace: resultSpace, finalCount: finalCount, firstView: prizContainerView as! UIImageView, secondView: prizImageView, gamesType: "addingFunLimit", containerView: UIView())
        delegate = self
    }
    ...
    private func drawImages(image: UIImage, view: UIView) {
        ...
        let count: Int = Random.getRandomIndex(in: 1..<6)
...
extension AddingFunController:handlePanGestureDelegate {
