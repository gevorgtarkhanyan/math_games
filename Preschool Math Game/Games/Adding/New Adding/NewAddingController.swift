//
//  NewAddingController.swift
//  Preschool Math Game
//
//  Created by Gago Mkrtchyan on 20.08.21.
//

import UIKit

class NewAddingController: BaseViewController {
    ...
    static func initWithStoryboard() -> NewAddingController? {
        let storyboard = UIStoryboard(name: "AddingGames", bundle: nil)
        return storyboard.instantiateViewController(withIdentifier: NewAddingController.name) as? NewAddingController
    }
    ...
    override func viewDidLoad() {
        ...
        //        AnalyticService.shared.setupAmplitude(game: "start_newAdding")
...
//        GamesLimit.newAddingPrizCount = UserDefaults.standard.value(forKey: UserDefaultsKeys.newAddingPrizCount) as? Int ?? 0

        NotificationCenter.default.addObserver(self, selector: #selector(update), name: .updateNewAdding, object: nil)
    ...
    private func setFirstLabelsCount() {
        let firstRandomNumber = 0..<5
        firstCounts = Random.getRandomIndex(in: firstRandomNumber)
        firstNum_1.text = String(firstCounts)

        let secondRandomNumber = 0..<5
        let secondCount: Int = Random.getRandomIndex(in: secondRandomNumber)
        firstNum_2.text = String(secondCount)
        firstFinalCount = firstCounts + secondCount
    }

    private func setSecondLabelsCount() {
        let firstRandomNumber = 0..<5
        secondCounts = Random.getRandomIndex(in: firstRandomNumber, ignore: firstCounts)
        secondNum_1.text = String(secondCounts)

        let secondRandomNumber = 0..<5
        let secondCount: Int = Random.getRandomIndex(in: secondRandomNumber)
        secondNum_2.text = String(secondCount)
        secondFinalCount = secondCounts + secondCount
    }

    private func setThirdLabelsCount() {
        let firstRandomNumber = 0..<5
        thirdCounts = Random.getRandomIndex(in: firstRandomNumber, ignoredIndexes: [firstCounts, secondCounts])
        thirdNum_1.text = String(thirdCounts)

        let secondRandomNumber = 0..<5
        let secondCount: Int = Random.getRandomIndex(in: secondRandomNumber)
        thirdNum_2.text = String(secondCount)
        thirdFinalCount = thirdCounts + secondCount
    }
        ...
        do {
            ...
            if let (label, _) = label(from: output) {
                ...
                if !UserDefaults.standard.bool(forKey: UserDefaultsKeys.paymentKey) {
                    if GamesLimit.newAddingLimit < 5 {
                        if gamesCount == 3 {
                            GamesLimit.newAddingLimit += 1
                            UserDefaults.standard.setValue(GamesLimit.newAddingLimit, forKey: UserDefaultsKeys.newAddingLimit)
                            if GamesLimit.newAddingLimit != 5 {
...
//                                GamesLimit.newAddingPrizCount += 1
//                                UserDefaults.standard.setValue(GamesLimit.newAddingPrizCount, forKey: UserDefaultsKeys.newAddingPrizCount)
                                game.isLocked = true
...
//                            UserDefaults.standard.setValue(GamesLimit.newAddingPrizCount, forKey: UserDefaultsKeys.newAddingPrizCount)
    ...
    private func scanImage(image: UIImageView) -> [Float]? {
        ...
        guard let image = image.image, let box = boundingBox else {
            return nil
        }
