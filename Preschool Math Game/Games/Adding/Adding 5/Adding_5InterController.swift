...
class Adding_5InterController: BaseViewController {
    ...
    static func initWithStoryboard() -> Adding_5InterController? {
        let storyboard = UIStoryboard(name: "AddingGames", bundle: nil)
        return storyboard.instantiateViewController(withIdentifier: Adding_5InterController.name) as? Adding_5InterController
    }
...
//        GamesLimit.adding_5PrizCount = UserDefaults.standard.value(forKey: UserDefaultsKeys.adding_5PrizCount) as? Int ?? 0
    ...
    private func setupButtons() {
        let max = 9
        var maxRez = 3
        var indexes = [result_1, result_2, result_3]
        
        if finalCount >= 1000 {
            maxRez = 2
            indexes.append(result_4)
        }
...
extension Adding_5InterController:handlePanGestureDelegate {
