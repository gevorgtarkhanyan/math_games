//
//  LearningFivethNumberController.swift
//  Preschool Math Game
//
...
class LearningFivethNumberController: BaseViewController {
        ...
        if gesture.state == .began {
            ...
            if index != 2 {
                if findNearestView(to: buttonOrigin) != nil {
                    ...
                    let x = fourthPoint.globalFrame.origin.x - 10
                    let y = fourthPoint.globalFrame.origin.y - 10
                    
                    UIView.animate(withDuration: 0.3) {
                        self.cloneImage.frame.origin = CGPoint(x: x, y: y)
                        self.numberImage.image = UIImage(named: "five_third")
                    }
                } else {
                ...
                if findNearestView(to: buttonOrigin) != nil {
                    ...
                    self.numberImage.image = UIImage(named: "five_fourth")
                    self.finalImage.forEach({$0.isHidden = true})
                    
                    if !UserDefaults.standard.bool(forKey: UserDefaultsKeys.paymentKey) {
                        if GamesLimit.learning_1Limit == 1 && GamesLimit.learning_2Limit == 1 && GamesLimit.learning_3Limit == 1 && GamesLimit.learning_4Limit == 1 && GamesLimit.learning_5Limit == 0 && GamesLimit.learning_6Limit == 1 && GamesLimit.learning_7Limit == 1 && GamesLimit.learning_8Limit == 1 && GamesLimit.learning_9Limit == 1 && GamesLimit.learning_10Limit == 1 {
...
//                            }
                            ...
                            NotificationCenter.default.post(name: .blockLearning_5, object: nil)
                            UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_5)

                            timer = Timer.scheduledTimer(withTimeInterval: 0.8, repeats: false) { timer in
                                self.navigationController?.popViewController(animated: true)
                            }
                        } else {
                            GamesLimit.learning_5Limit = 1
                            UserDefaults.standard.setValue(GamesLimit.learning_5Limit, forKey: UserDefaultsKeys.learning_5Limit)
                            NotificationCenter.default.post(name: .blockLearning_5, object: nil)
                            UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_5)
    ...
    @objc func firstPan(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            if let nearestView = findNearestView(to: buttonOrigin) {
                ...
                let x = nearestView.globalFrame.origin.x - 10
                let y = nearestView.globalFrame.origin.y - 8
                
                UIView.animate(withDuration: 0.3) {
                    self.cloneImage.frame.origin = CGPoint(x: x, y: y)
                    self.numberImage.image = UIImage(named: "five_second")
                }
            } else {
                button.transform = .identity
            }
