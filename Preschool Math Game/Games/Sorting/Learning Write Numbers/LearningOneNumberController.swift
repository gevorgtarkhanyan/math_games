//
//  LearningOneNumberController.swift
//  Preschool Math Game
//
//  Created by Gago Mkrtchyan on 16.08.21.
//

import UIKit

class LearningOneNumberController: BaseViewController {
    
    //MARK: - Outlets
    @IBOutlet weak var drop_pink: UIImageView!
    @IBOutlet weak var firstPoint: UIView!
    @IBOutlet weak var secondPoint: UIView!
    @IBOutlet var pointCollection: [UIView]!
    ...
    @objc func secondPan(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            if findNearestView(to: buttonOrigin) != nil {
                ...
                self.numberImage.image = UIImage(named: "one_third")
                self.finalImage.forEach({$0.isHidden = true})
                
                if !UserDefaults.standard.bool(forKey: UserDefaultsKeys.paymentKey) {
                    if GamesLimit.learning_1Limit == 0 && GamesLimit.learning_2Limit == 1 && GamesLimit.learning_3Limit == 1 && GamesLimit.learning_4Limit == 1 && GamesLimit.learning_5Limit == 1 && GamesLimit.learning_6Limit == 1 && GamesLimit.learning_7Limit == 1 && GamesLimit.learning_8Limit == 1 && GamesLimit.learning_9Limit == 1 && GamesLimit.learning_10Limit == 1 {
...
//                        UserDefaults.standard.setValue(GamesLimit.learningPrizCount, forKey: UserDefaultsKeys.learningPrizCount)
                        ...
                        NotificationCenter.default.post(name: .blockLearning_1, object: nil)
                        UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_1)

                        timer = Timer.scheduledTimer(withTimeInterval: 0.8, repeats: false) { timer in
                            self.navigationController?.popViewController(animated: true)
                        }
                    } else {
                        GamesLimit.learning_1Limit = 1
                        UserDefaults.standard.setValue(GamesLimit.learning_1Limit, forKey: UserDefaultsKeys.learning_1Limit)
                        NotificationCenter.default.post(name: .blockLearning_1, object: nil)
                        UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_1)
    ...
    @objc func firstPan(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            if let nearestView = findNearestView(to: buttonOrigin) {
                ...
                setCloneImage(image: button, point: buttonOrigin, btnWidth: 45, btnHeight: 45)
                ...
                UIView.animate(withDuration: 0.3) {
                    ...
                    self.numberImage.image = UIImage(named: "one_second")
                }
            } else {
                button.transform = .identity
            }
