//
//  LearningNinethNumberController.swift
//  Preschool Math Game
//
...
class LearningNinethNumberController: BaseViewController {
    ...
    @objc func secondPan(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            if findNearestView(to: buttonOrigin) != nil {
                ...
                self.numberImage.image = UIImage(named: "nine_third")
                self.finalImage.forEach({$0.isHidden = true})
                
                if !UserDefaults.standard.bool(forKey: UserDefaultsKeys.paymentKey) {
                    if GamesLimit.learning_1Limit == 1 && GamesLimit.learning_2Limit == 1 && GamesLimit.learning_3Limit == 1 && GamesLimit.learning_4Limit == 1 && GamesLimit.learning_5Limit == 1 && GamesLimit.learning_6Limit == 1 && GamesLimit.learning_7Limit == 1 && GamesLimit.learning_8Limit == 1 && GamesLimit.learning_9Limit == 0 && GamesLimit.learning_10Limit == 1 {
...
//                        }
                        ...
                        NotificationCenter.default.post(name: .blockLearning_9, object: nil)
                        UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_9)

                        timer = Timer.scheduledTimer(withTimeInterval: 0.8, repeats: false) { timer in
                            self.navigationController?.popViewController(animated: true)
                        }
                    } else {
                        GamesLimit.learning_9Limit = 1
                        UserDefaults.standard.setValue(GamesLimit.learning_9Limit, forKey: UserDefaultsKeys.learning_9Limit)
                        NotificationCenter.default.post(name: .blockLearning_9, object: nil)
                        UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_9)
