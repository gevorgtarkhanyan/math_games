//
//  LearningTenthNumberController.swift
//  Preschool Math Game
//
...
class LearningTenthNumberController: BaseViewController {
    ...
    private func findNearestView(to point: CGPoint) -> UIView? {
        ...
        if index == 0 {
            x = firstPoint.globalFrame.maxX
            y = firstPoint.globalFrame.minY
            finalView = firstPoint
        } else if index == 1 {
            x = secondPoint.globalFrame.maxX
            y = secondPoint.globalFrame.minY
            finalView = secondPoint
        } else if index == 2 {
            x = thirdPoint.globalFrame.maxX
            y = thirdPoint.globalFrame.minY
            finalView = thirdPoint
        } 
        
        if abs(point.x - x) < inset && abs(point.y - y) < inset {
            nearestView = finalView
        }
    ...
    @objc func secondPan(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            let secondOrigin = fourthPoint.globalFrame.origin
            ...
            if index == 1 {
                if findNearestView(to: buttonOrigin) != nil {
                    ...
                    setCloneImage(image: button, point: secondOrigin, btnWidth: button.frame.width, btnHeight: button.frame.height)
                    
                    let x = secondOrigin.x - 10
                    let y = secondOrigin.y - 15
                    
                    UIView.animate(withDuration: 0.3) {
                        self.cloneImage.frame.origin = CGPoint(x: x, y: y)
                        
                        self.numberImage.image = UIImage(named: "ten_third")
                    }
                } else {
                ...
                if findNearestView(to: buttonOrigin) != nil {
                    ...
                    self.numberImage.image = UIImage(named: "ten_fourth")
                    self.finalImage.forEach({$0.isHidden = true})
                    
                    if !UserDefaults.standard.bool(forKey: UserDefaultsKeys.paymentKey) {
                        if GamesLimit.learning_1Limit == 1 && GamesLimit.learning_2Limit == 1 && GamesLimit.learning_3Limit == 1 && GamesLimit.learning_4Limit == 1 && GamesLimit.learning_5Limit == 1 && GamesLimit.learning_6Limit == 1 && GamesLimit.learning_7Limit == 1 && GamesLimit.learning_8Limit == 1 && GamesLimit.learning_9Limit == 1 && GamesLimit.learning_10Limit == 0 {
...
//                            }
                            ...
                            NotificationCenter.default.post(name: .blockLearning_10, object: nil)
                            UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_10)

                            timer = Timer.scheduledTimer(withTimeInterval: 0.8, repeats: false) { timer in
                                self.navigationController?.popViewController(animated: true)
                            }
                        } else {
                            GamesLimit.learning_10Limit = 1
                            UserDefaults.standard.setValue(GamesLimit.learning_10Limit, forKey: UserDefaultsKeys.learning_10Limit)
                            NotificationCenter.default.post(name: .blockLearning_10, object: nil)
                            UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_10)
...
//                            UserDefaults.standard.setValue(GamesLimit.learningPrizCount, forKey: UserDefaultsKeys.learningPrizCount)
    ...
    @objc func firstPan(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            if let nearestView = findNearestView(to: buttonOrigin) {
                
                index = 1
                
                setCloneImage(image: button, point: buttonOrigin, btnWidth: button.frame.width, btnHeight: button.frame.height)
                
                let x = nearestView.globalFrame.origin.x - 3
                let y = nearestView.globalFrame.origin.y - 10
                
                UIView.animate(withDuration: 0.3) {
                    self.cloneImage.frame.origin = CGPoint(x: x, y: y)
                    self.numberImage.image = UIImage(named: "ten_second")
                }
            } else {
                button.transform = .identity
            }
