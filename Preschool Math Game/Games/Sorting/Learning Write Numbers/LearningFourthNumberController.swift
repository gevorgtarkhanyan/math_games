//
//  LearningFourthNumberController.swift
//  Preschool Math Game
//
...
class LearningFourthNumberController: BaseViewController {
        ...
        if gesture.state == .began {
            ...
            if index != 2 {
                ...
                if findNearestView(to: buttonOrigin) != nil {
                    ...
                    self.numberImage.image = UIImage(named: "four_fourth")
                    self.finalImage.forEach({$0.isHidden = true})
                    
                    if !UserDefaults.standard.bool(forKey: UserDefaultsKeys.paymentKey) {
                        if GamesLimit.learning_1Limit == 1 && GamesLimit.learning_2Limit == 1 && GamesLimit.learning_3Limit == 1 && GamesLimit.learning_4Limit == 0 && GamesLimit.learning_5Limit == 1 && GamesLimit.learning_6Limit == 1 && GamesLimit.learning_7Limit == 1 && GamesLimit.learning_8Limit == 1 && GamesLimit.learning_9Limit == 1 && GamesLimit.learning_10Limit == 1 {
...
//                            }
                            ...
                            GamesLimit.learning_4Limit = 1
                            UserDefaults.standard.setValue(GamesLimit.learning_4Limit, forKey: UserDefaultsKeys.learning_4Limit)
                            NotificationCenter.default.post(name: .blockLearning_4, object: nil)
                            UserDefaults.standard.set(true, forKey: UserDefaultsKeys.isLockLearning_4)
    ...
    @objc func firstPan(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            if let nearestView = findNearestView(to: buttonOrigin) {
                ...
                let y = nearestView.globalFrame.origin.y - 15
                
                UIView.animate(withDuration: 0.3) {
                    self.cloneImage.frame.origin = CGPoint(x: x, y: y)
                    self.numberImage.image = UIImage(named: "four_second")
                }
            } else {
                button.transform = .identity
            }
