//
//  NewDivisionController.swift
//  Preschool Math Game
//
//  Created by Gago Mkrtchyan on 24.08.21.
//

import UIKit

class NewDivisionController: BaseViewController {
    ...
    static func initWithStoryboard() -> NewDivisionController? {
        let storyboard = UIStoryboard(name: "DivisionGames", bundle: nil)
        return storyboard.instantiateViewController(withIdentifier: NewDivisionController.name) as? NewDivisionController
    }
    ...
    override func viewDidLoad() {
        ...
        //        AnalyticService.shared.setupAmplitude(game: "start_newDivision")
...
//        GamesLimit.newDivisionPrizCount = UserDefaults.standard.value(forKey: UserDefaultsKeys.newDivisionPrizCount) as? Int ?? 0

        NotificationCenter.default.addObserver(self, selector: #selector(update), name: .updateNewDivision, object: nil)
    ...
    private func setFirstLabelsCount() {
        ...
        let num = baseNum[0]
        firstCounts = Random.getRandomNumber(in: num, ignoredIndexes: [secondCounts, thirdCounts])
        firstNum_1.text = String(firstCounts)

        if firstNum_1.text == "2" {
            firstNum_2.text = String(_2[0])
        } else if firstNum_1.text == "4" {
            firstNum_2.text = String(_4[0])
        } else if firstNum_1.text == "6" {
            firstNum_2.text = String(_6[0])
        } else if firstNum_1.text == "8" {
            firstNum_2.text = String(_8[0])
        }

        firstFinalCount = Int(firstNum_1.text!)! / Int(firstNum_2.text!)!
    }
    ...
    private func setSecondLabelsCount() {
        ...
        let num = baseNum[0]
        secondCounts = Random.getRandomNumber(in: num, ignoredIndexes: [firstCounts])
        secondNum_1.text = String(secondCounts)

        if secondNum_1.text == "2" {
            secondNum_2.text = String(_2[0])
        } else if secondNum_1.text == "4" {
            secondNum_2.text = String(_4[0])
        } else if secondNum_1.text == "6" {
            secondNum_2.text = String(_6[0])
        } else if secondNum_1.text == "8" {
            secondNum_2.text = String(_8[0])
        }

        secondFinalCount = Int(secondNum_1.text!)! / Int(secondNum_2.text!)!
    }
    ...
    private func setThirdLabelsCount() {
        ...
        let num = baseNum[0]
        thirdCounts = Random.getRandomNumber(in: num, ignoredIndexes: [firstCounts, secondCounts])
        thirdNum_1.text = String(thirdCounts)

        if thirdNum_1.text == "2" {
            thirdNum_2.text = String(_2[0])
        } else if thirdNum_1.text == "4" {
            thirdNum_2.text = String(_4[0])
        } else if thirdNum_1.text == "6" {
            thirdNum_2.text = String(_6[0])
        } else if thirdNum_1.text == "8" {
            thirdNum_2.text = String(_8[0])
        }

        thirdFinalCount = Int(thirdNum_1.text!)! / Int(thirdNum_2.text!)!
    }
...
// MARK: - Touch in View
extension NewDivisionController {
        ...
        do {
            ...
            if let (label, _) = label(from: output) {
                ...
                if !UserDefaults.standard.bool(forKey: UserDefaultsKeys.paymentKey) {
                    if GamesLimit.newDivisionLimit < 5 {
                        if gamesCount == 3 {
                            GamesLimit.newDivisionLimit += 1
                            UserDefaults.standard.setValue(GamesLimit.newDivisionLimit, forKey: UserDefaultsKeys.newDivisionLimit)
...
//                                UserDefaults.standard.setValue(GamesLimit.newDivisionPrizCount, forKey: UserDefaultsKeys.newDivisionPrizCount)
                                game.isLocked = true
