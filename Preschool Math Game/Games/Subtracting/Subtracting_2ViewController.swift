//
//  Subtracting_2ViewController.swift
//  Preschool Math Game
//
...
class Subtracting_2ViewController: BaseViewController {
    ...
    static func initWithStoryboard() -> Subtracting_2ViewController? {
        let storyboard = UIStoryboard(name: "SubtractingGames", bundle: nil)
        return storyboard.instantiateViewController(withIdentifier: Subtracting_2ViewController.name) as? Subtracting_2ViewController
    }
...
//        AnalyticService.shared.setupAmplitude(game: "start_subtracting_2")
        
        setBorders()
        
//        GamesLimit.subtractingFun_2PrizCount = UserDefaults.standard.value(forKey: UserDefaultsKeys.subtracting_2PrizCount) as? Int ?? 0
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(update), name: .updateSubtracting_2, object: nil)
    }
    ...
    @objc func update() {
        ...
        GamesLimit.subtracting_2Limit = 0
    ...
    @objc func gestureButtonAction(gesture: UIPanGestureRecognizer) {
        threeButtonPanGesture(gesture: gesture, gamesLimit: GamesLimit.subtracting_2Limit, resultSpace: resultSpace, finalCount: finalCount, firstView: prizContainerView as! UIImageView, secondView: prizImageView, gamesType: "subtracting_2Limit", containerView: UIView())
        delegate = self
    }
...
extension Subtracting_2ViewController:handlePanGestureDelegate {
